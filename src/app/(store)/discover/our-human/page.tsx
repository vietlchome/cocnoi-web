...
export default async function OurHumanPage() {
  const config = await getSiteConfig();
  const s = config.discover_human;

  const eyebrow       = s?.eyebrow       || "Our Human";
  const title         = s?.title         || "Những đôi tay làm nên Cốc Nối";
  const subtitle      = s?.subtitle      || "Mỗi đôi cốc đi qua nhiều đôi tay. Đây là những con người dệt nên Cốc Nối, từ đất sét thô sơ đến bàn tay nâng niu của bạn.";
  const members       = s?.members?.length ? s.members : DEFAULT_MEMBERS;
  const memberBadge   = s?.memberBadge   || "Nghệ nhân Bát Tràng truyền thống";
  const ctaHeading    = s?.ctaHeading    || "Đồng hành cùng Cốc Nối";
  const ctaText       = s?.ctaText       || "Chúng tôi luôn tìm kiếm những cửa hàng kí gửi, đối tác phân phối và các doanh nghiệp muốn chia sẻ câu chuyện gốm thủ công ý nghĩa đến khách hàng.";
  const ctaButtonText = s?.ctaButtonText || "Tìm hiểu cơ hội hợp tác";
  const ctaButtonHref = s?.ctaButtonHref || "/partners";
    ...
    <main className="w-full bg-canvas text-primary">
      ...
      <section className="py-20 md:py-24 max-w-5xl mx-auto px-4 md:px-8">
        ...
        <div className="mt-20 text-center max-w-xl mx-auto border-t border-border/40 pt-16">
          ...
          <h3 className="font-playfair text-2xl font-bold text-primary mb-3">{ctaHeading}</h3>
