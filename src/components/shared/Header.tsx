...
export default function Header() {
  ...
  const [legacySettings, setLegacySettings] = useState<Record<string, string>>({});
  ...
  // Các biến cấu hình từ SiteConfig
  const headerLogoUrl = themeConfig?.header?.logoUrl || "";
  ...
  const menuSetting = legacySettings["menu_items"] || "";
  try {
  ...
  return (
    <div className="w-full flex flex-col z-50 relative">
      ...
      }`} style={{ backgroundColor: isScrolled ? undefined : "var(--color-warm-white)" }}>
              ...
              return (
                <div key={link.title} className="relative group/item py-2">
                  <Link 
                    ...
                    style={{ 
                      color: pathname === link.link ? "var(--color-terracotta)" : "var(--color-deep-indigo)",
                      borderBottomColor: pathname === link.link ? "var(--color-terracotta)" : "transparent"
                    }}
                  >
                    {link.title}
