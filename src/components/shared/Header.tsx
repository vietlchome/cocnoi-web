...
export default function Header() {
  ...
  // Nạp cài đặt động từ Database qua API
  ...
  useEffect(() => {
    const fetchHeaderSettings = async () => {
      try {
        ...
        // Fetch unique collections dynamically from DB products
        const dataRes = await fetch("/api/admin/data");
        if (dataRes.ok) {
          const data = await dataRes.json();
          if (data.success && data.products) {
            const collections = Array.from(
              new Set(data.products.map((p: any) => p.collection).filter(Boolean))
            ) as string[];
            setDbCollections(collections);
          }
        }
      } catch (e) {
        console.error("Lỗi khi nạp cấu hình Header:", e);
      }
    };
    fetchHeaderSettings();
  }, []);
  ...
  const menuSetting = getSetting("menu_items", "");
  try {
  ...
  // Bản đồ submenu tĩnh khớp theo nhãn tiêu đề
  ...
  const isSticky = getSetting("sticky_header", "true") === "true";

  return (
    <div className="w-full flex flex-col z-50 relative">
      
      {/* 1. TOP BAR ĐỘNG */}
      {getSetting("show_top_bar", "true") === "true" && (
        <div 
        ...
        >
          {getSetting("top_bar_text", "Miễn phí vận chuyển toàn quốc cho đơn hàng trên 1.000.000 đ")}
        </div>
      )}

      {/* 2. MAIN HEADER DYNAMICS */}
      <header className={`${isSticky ? "sticky top-0" : "relative"} transition-all duration-300 w-full ${
      ...
      }`} style={{ backgroundColor: isScrolled ? undefined : getSetting("bg_color", "#FEFCF9") }}>
        ...
        <div className="max-w-[1280px] mx-auto px-4 md:px-8 flex items-center justify-between">
          ...
          <Link href="/" className="flex items-center gap-3 group">
            {getSetting("logo_image_url", "") ? (
              ...
              <>
                ...
                <span className="font-playfair text-xl md:text-2xl font-semibold tracking-wider text-primary select-none group-hover:text-accent transition-colors" style={{ color: getSetting("primary_color", "#131829") }}>
              ...
              return (
                <div key={link.title} className="relative group/item py-2">
                  <Link 
                    ...
                    style={{ 
                      color: pathname === link.link ? getSetting("accent_color", "#C2703E") : getSetting("primary_color", "#131829"),
                      borderBottomColor: pathname === link.link ? getSetting("accent_color", "#C2703E") : "transparent"
                    }}
                  >
                    {link.title}
                  ...
                  {link.title.toUpperCase() !== "SHOP" && submenu && (
                    <div className="absolute top-full left-1/2 -translate-x-1/2 mt-1 w-56 bg-canvas border border-border rounded-3 shadow-lg opacity-0 translate-y-2 pointer-events-none group-hover/item:opacity-100 group-hover/item:translate-y-0 group-hover/item:pointer-events-auto transition-all duration-300 z-50 p-2">
                      <div className="py-1">
                        {submenu.map((sub) => (
                          <Link
                            key={sub.name}
          ...
          {/* UTILITIES AREA */}
          <div className="flex items-center gap-3 md:gap-5" style={{ color: getSetting("primary_color", "#131829") }}>
            {/* Lang Toggle */}
            <button 
            ...
            >
              ...
              {cartCount > 0 ? (
                ...
                <span className="absolute top-0 right-0 w-2.5 h-2.5 bg-accent rounded-full animate-pulse border border-canvas" style={{ backgroundColor: getSetting("accent_color", "#C2703E") }}></span>
              )}
            </button>
        ...
        {/* MOBILE DRAWER */}
        {isOpen && (
          <div className="lg:hidden absolute top-full left-0 w-full bg-canvas border-b border-border shadow-xl z-50 animate-fade-in">
